...
(function () {
  ...
  var DIY_SIZE_OPTION = {
    ...
    choices: SIZE_CHOICES,
    default: "1200x2200",
    note: "천장 높이를 좌·중·우 3곳에서 재고, 가장 낮은 값이 '적합 천장고' 범위에 드는 사이즈를 선택하세요. 3곳 편차가 15mm를 넘으면 유상 출장 실측을 이용해 주세요. 실제 개구부(가로·세로)보다 작은 사이즈는 조절볼트로 보정해 설치할 수 있지만, 큰 사이즈는 설치 자체가 불가능하니 반드시 실측값보다 작거나 같은 사이즈를 선택하세요.",
  };
  ...
  // 자동문 — 4가지 구조(3연동/원슬라이딩/스윙폴딩/여닫이) 중 하나에 얹는 자동 개폐 추가 기능.
  // 별도 구조가 아니라 옵션 선택 최하단의 addon 체크로 다룬다(2026-08-16, 파티션과 동일한 방식).
  var AUTO_DOOR_ADDON_OPTION = {
    id: "autoAddon",
    kind: "addon",
    label: "자동문 추가",
    description: "감지센서로 자동 개폐되는 기능을 추가합니다. 선택한 구조(3연동·원슬라이딩·스윙폴딩·여닫이)에 적용됩니다.",
    default: false,
  };
  ...
  //         ② 프레임 색상은 도장/필름 × 화이트/그레이/블랙 6종 노출(2단 선택). 미리보기에서
  //            프레임(A) 레이어를 그리지 않으므로 이미지 자산 유무와 무관하게 전부 선택 가능.
  //         ③ 자동문은 문 유형이 아니라 하단 "추가 옵션" 체크(addons)로 다룬다.
  var LAYERED_CUSTOMIZER_HEDAUM = {
    ...
    types: [
      ...
      // 자동문은 문 유형이 아니라 파티션과 동일하게 하단 "추가 옵션" 체크로 이동(2026-08-16).
    ],
    // 옵션 선택 최하단 추가 옵션(체크형). 파티션은 예시 이미지 보기 버튼 동반.
    addons: [
      { value: "partition", label: "파티션 추가", hasExample: true },
      { value: "auto", label: "자동문 추가" },
    ],
  ...
  window.MUTAGONG_PRODUCTS = {
    "standard-door": {
      ...
      category: "중문",
      summary:
        "혜다움의 시공형 중문입니다. 레일과 힌지로 시공하는 일반적인 방식으로, 3연동·원슬라이딩·스윙폴딩·여닫이 구조로 제작하며(자동문은 이 중 한 구조에 얹는 추가 옵션) 지금 바로 상담과 구매 문의가 가능합니다.",
      features: [
        { title: "4가지 구조", desc: "3연동·원슬라이딩·스윙폴딩·여닫이 중 공간에 맞는 구조를 선택할 수 있습니다. 자동문은 선택한 구조에 추가하는 옵션입니다." },
      ...
      options: [
        ...
        AUTO_DOOR_ADDON_OPTION,
      ],
      purchaseNotice:
        "본 상품은 시공팀이 서울·경기·경상권 기준 무료로 현장 방문 실측을 진행한 뒤 제작·시공에 들어가는 시공형 제품입니다. 그 외 지역은 무료 실측이 어려워 별도 비용이 발생하며, 정확한 금액은 상담 시 안내드립니다. 실측 예약 후 방문 전에 취소하실 경우 전액 환불되며, 실측 후 제작 시작 전에 취소하실 경우 3만원을 제외하고 환불됩니다. 제작이 시작된 이후에는 단순 변심에 의한 취소·반품이 불가합니다. 제품 또는 시공 하자가 확인되는 경우 구매일로부터 1년간 무상 A/S가 적용됩니다(고객 과실로 인한 하자는 비용이 발생할 수 있습니다). A/S 출장비는 실측과 동일한 기준으로 안내드립니다.",
    },
    ...
    "diy-door": {
      ...
      features: [
        ...
        { title: "구조 안전성 자체 검토", desc: "하중과 고정력을 자체적으로 검토해 안전성을 확인해 나가고 있습니다. (제3자 공인 시험·인증은 준비 중이며 아직 완료되지 않았습니다.)" },
      ],
      useCases: [
      ...
      purchaseNotice:
        "본 상품은 고객님이 직접 실측하신 값에 맞춰 개별로 재단·제작되는 셀프 시공 제품입니다. 실측이 어려우시면 지역에 따라 3~10만원의 비용으로 출장 실측을 요청하실 수 있으며, 실측 전 취소는 전액 환불, 실측 후·생산 시작 전 취소는 실측 비용을 제외하고 환불됩니다. 다만 생산이 시작된 이후에는 단순 변심에 의한 취소·반품이 불가합니다. 제품 하자가 확인되는 경우 구매일로부터 1년간 무상 A/S가 적용되며, A/S 접수 시 배송비는 고객님 부담입니다.",
    },
  ...
  window.MUTAGONG_BRAND_GROUPS = [
    ...
    },
    {
      ...
      items: [
        ...
        // 자동문은 구조가 아니라 addon이라 여기서 제외(2026-08-16) — 4구조 중 하나를 고른 뒤
        // 옵션 선택 최하단 "자동문 추가" 체크로 선택한다.
      ],
    },
  ];
})();
