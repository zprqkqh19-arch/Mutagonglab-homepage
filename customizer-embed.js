...
(function(){
  ...
  mkGroup($('gSize'),'size',SIZES.map(function(s){return [s[0],s[0]];}));
  ...
  $('consultBtn').onclick=function(){location.href='contact.html?brand=mutagonglab&sku='+encodeURIComponent($('sku').textContent);};
  var canvas=$('canvas');
  var ticks=[];
  function mkTicks(){
    ticks.forEach(function(t){t.remove();});ticks=[];
    var sz=SIZE_MAP[S.size]||SIZE_MAP['12-22'];
  ...
  function render(){
    ...
    $('aImg').src=(S.size==='12-22')?SK('skus/12-22/A/A_12-22_sv'+(S.ext?'_ext':'')+'.svg'):('data:image/svg+xml;charset=utf-8,'+encodeURIComponent(frameSVG(W,totalH,S.ext)).replace(/'/g,'%27'));
    ...
    ov.innerHTML=svg;
    $('dimW').textContent='W '+W;$('dimW').style.top=(totalH*SCd+4)+'px';
    ...
    ['gType','gSub','gFrame','gDoor','gGlass','gHandle','gSize'].forEach(function(id){
      var key={gType:'t',gSub:'sub',gFrame:'a',gDoor:'d',gGlass:'g',gHandle:'h',gSize:'size'}[id];
    ...
    var rows=[['제품 유형',S.t+(S.t==='여닫이'?' · '+NM.sub[S.sub]:'')],['설치 규격','W'+W+' × H'+totalH+' ('+S.size+(S.ext?', +30mm 연장':'')+')'],['중문 색상',NM.d[S.d]],['안전창',NM.g[S.g]],['간살',gansalTxt.length?gansalTxt.join(' · '):'없음'],['손잡이',NM.h[S.h]]];
